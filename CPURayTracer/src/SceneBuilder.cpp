        ...
        while (scenefile) {
            ...
            if ((line.find_first_not_of(" \t\r\n") != string::npos) && (line[0] != '#')) {
                ...
                if (cmd == "size") { // Set Image Size.
                    bool validinput = this->readCommandParameters(cmdParams, 2, params); // w, h 
                    if (validinput) {
                        options.ImageWidth = params[0];
                        options.ImageHeight = params[1];
                    }
                }
                else if (cmd == "maxdepth") { // Set Ray Depth.
                    bool validinput = this->readCommandParameters(cmdParams, 1, params); // depth 
                    if (validinput) {
                        options.MaxRayDepth = (int)params[0];
                    }
                }
                else if (cmd == "output") { // Set Output File Name.
                    cmdParams >> options.OutputFileName;
                }
                else if (cmd == "camera") { // Add Camera to Scene.
                    bool validinput = this->readCommandParameters(cmdParams, 10, params); // pos, eye, up, fov 
                    if (validinput) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 4, params); // pos, radius 
                    if (validinput) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 3, params); // pos 
                    if (validinput) {
                        vertices.push_back(pair<vec4,vec4>(vec4(params[0], params[1], params[2], 1), vec4(0.0)));
                    }
                }
                else if (cmd == "vertexnormal") { // Store Normal w/ Vertex.
                    bool validinput = this->readCommandParameters(cmdParams, 6, params); // pos, normal 
                    if (validinput) {
                        vec4 vert(params[0], params[1], params[2], 1);
                        vec4 norm(params[3], params[4], params[5], 1);
                        vertices.push_back(pair<vec4, vec4>(vert, norm));
                    }
                }
                else if (cmd == "tri") { // Store Triangle.
                    bool validinput = this->readCommandParameters(cmdParams, 3, params); // v1, v2, v3 
                    if (validinput) {
                        Triangle tri = Triangle(vertices[(int)params[0]].first, vertices[(int)params[1]].first, vertices[(int)params[2]].first);
                        tri.setNormals(vertices[(int)params[0]].second, vertices[(int)params[1]].second, vertices[(int)params[2]].second);
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 7, params); // x, y, z, r, g, b, a 
                    if (validinput) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 3, params); // const, linear, quadratic 
                    if (validinput) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 3, params); // r, g, b 
                    if (validinput) {
                        for (int i = 0; i < 3; i++) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 1, params); // s
                    if (validinput) {
                        currShininess = params[0];
                    }
                }
                else if (cmd == "translate") { // Below are Transformations.
                    bool validinput = this->readCommandParameters(cmdParams, 3, params); // x, y, z
                    if (validinput) {
                    ...
                    bool validinput = this->readCommandParameters(cmdParams, 4, params); // x, y, z, angle
                    if (validinput) {
